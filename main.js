...
import {
  ...
  updateBestScoreIfHigher, USERNAME_MAX_LENGTH,
} from './profile.js';
...
function wireAikotobaModal(){
  const openBtn = document.getElementById('aikotobaBtn');
  const closeBtn = document.getElementById('aikotobaCloseBtn');
  const submitBtn = document.getElementById('aikotobaSubmitBtn');
  if(openBtn) openBtn.addEventListener('click', ()=>{ Sound.click(); openModal('aikotobaModal'); });
...
async function onAikotobaSubmit(){
  ...
  if(result.success){
    ...
    Game.addCoins(result.coinAmount);
    showToast(result.message, { title:'あいことば成功！', variant:'success' });
    input.value = '';
